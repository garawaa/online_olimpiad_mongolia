...
vector< rect > ehlel[MAXN];
vector< rect > togsgol[MAXN];
  ...
  for (int i = M - 1; i >= 0; --i) {
    switch (command[i]) {
      ...
      case NEEH:
        ...
        break;
      case ZURAH:
        ehlel[paint[i].x1].push_back(paint[i]);
        togsgol[paint[i].x2 + 1].push_back(paint[i]);
    }
  }

  for (int row = 0; row < N; ++row, odd ^= 1) {
    for (int i = 0; i < togsgol[row].size(); ++i)
      T.remove(togsgol[row][i]);
    for (int i = 0; i < ehlel[row].size(); ++i)
      T.insert(ehlel[row][i]);
