...
struct pq_cmpf {
    bool operator () ( int a, int b ) const {
        ...
        for( int bit = 9; bit >= 0; --bit ) {
            ...
            if( rank1[a][poca][bit] != rank1[b][pocb][bit] ) 
                return rank1[a][poca][bit] < rank1[b][pocb][bit];
            else {
                poca += (1<<bit);
                pocb += (1<<bit);
            }
    ...
    for( int b = 0; (1<<b) <= 1000; ++b ) {
        ...
        for( int i = 0; i < n; ++i ) 
            for( int j = 0; j+len <= L[i]; ++j ) {
                ...
                if( len == 1 ) { Val = A[i][j]; }
                else { Val = rank1[i][j][b-1]*100000100LL + rank1[i][j+pola][b-1]; }
